...
def dot_product_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    rowdict = mat2rowdict(M) # dict of row labels to row vectors.
    return Vec(M.D[0], {rowlabel : v*rowdict[rowlabel] for rowlabel in M.D[0]})
...
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    coldict = mat2coldict(M) # dict of col labels to col vectors
    return Vec(M.D[1], {collabel : v*coldict[collabel] for collabel in M.D[1]})
...
def Mv_mat_mat_mult(A, B):
    ...
    return coldict2mat({Bcollabel : A*Bcolvec for Bcollabel, Bcolvec in Bcoldict.items()})
...
def vM_mat_mat_mult(A, B):
    ...
    return rowdict2mat({Arowlabel: Arowvec*B for Arowlabel, Arowvec in Arowdict.items()})
