...
from bitutil import noise,str2bits,bits2mat
from GF2 import one
...
def find_error(e):
    ...
    """
    hcoldict = mat2coldict(H)
    index = None
    for collabel, colvec in hcoldict.items():
        if colvec == e:
            index=collabel
    resultdict = {} if index==None else {index:one}
    return Vec({0, 1, 2, 3, 4, 5, 6}, resultdict)
...
original = list2vec([0,one,0,one]) # R * code_word
...
def find_error_matrix(S):
    ...
    """
    return coldict2mat({collabel:find_error(colvec) for collabel, colvec in mat2coldict(S).items()})
